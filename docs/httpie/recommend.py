...
def get_apigw_addr():
    ...
    return cfn.describe_stacks(StackName="PersonalizePipelineDemoDevApiGatewayStack")[
        "Stacks"
...
def recommend_similar_items(base_url, item_id, num_results=10):
    if not SIMILAR_ITEMS_ARN:
        logging.error(
            "SIMILAR_ITEMS_ARN should be setted to invoke recommend_similar_items. use `export SIMILAR_ITEMS_ARN` in terminal"
        )
        return []
...
def recommend_user_personalization(base_url, user_id, num_results=10):
    if not USER_PERSONALIZATION_ARN:
        logging.error(
            "USER_PERSONALIZATION_ARN should be setted to invoke recommend_user_personalization. use `export USER_PERSONALIZATION_ARN` in terminal"
        )
        return []
...
if __name__ == "__main__":
    ...
    if items:
        logging.info(
            "SimliarItems: \n{}".format(
                json.dumps(convert_item_to_movies(movies, items), indent=2)
            )
        )
