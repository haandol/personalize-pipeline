...
def handler(event, context):
    ...
    if not (bucket or user_bucket):
        raise RuntimeError(
            "at least one of item_bucket or user_bucket should be provided"
        )

    if bucket:
        if not bucket.startswith("s3://") and not bucket.endswith(".csv"):
            raise Exception(
                f"Invalid bucket format, s3://BUCKET_NAME/XYZ.csv but {bucket}"
            )
        if not schema_arn:
    ...
    if user_bucket:
        if not user_bucket.startswith("s3://") and not user_bucket.endswith(".csv"):
            raise Exception(
                f"Invalid bucket format, s3://BUCKET_NAME/XYZ.csv but {user_bucket}"
            )
